...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='coco', help='', choices=['coco', 'voc'])
...
def main():
    ...
    if args.dataset in ['voc']:
        ...
        name_template = 'frcn_{}_r101_novel_{}shot_seedx.yaml'
        yaml_path = os.path.join(args.config_root, name_template.format(args.setting, args.shot))
        yaml_info = load_config_file(yaml_path)
        for i, lineinfo in enumerate(yaml_info):
            if '  TRAIN: ' in lineinfo:
                # coco_trainval_all_10shot
                _str_ = '  TRAIN: ("coco_trainval_{}_{}shot_seed{}", )\n'
